...
from kaggle_agent.knowledge import ensure_knowledge_layout, read_knowledge_context
...
def _candidate_queries(root_cause: str, family: str) -> list[str]:
    text = f"{root_cause}\n{family}".lower()
    queries: list[str] = []
...
def _default_research_payload(root_cause: str, family: str, knowledge_context: str) -> dict[str, object]:
    queries = _candidate_queries(root_cause, family)
    adopt_now: list[str] = []
    consider: list[str] = []
    reject: list[str] = []
    lower_context = knowledge_context.lower()
    if "holdout" in lower_context and "proxy" in lower_context:
        adopt_now.append("keep separating source holdout metrics from target-facing proxy metrics")
    if "probe" in family:
        adopt_now.append("expand the cached-probe grid before moving to heavier finetuning")
        consider.append("blend probe priors with simple soundscape-aware post-processing")
    ...
    return {
        ...
        "consider": consider,
        "reject": reject,
        "knowledge_files_seen": knowledge_context.count("\n## "),
    }
